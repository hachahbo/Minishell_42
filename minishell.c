...
void	parser(t_list *head, t_env *env_list, char *input, int flag)
{
	t_list	*new_list;
	t_list	*new_list_w_s;
	t_list	*last_list;
	...
	if (ft_valid_command(head) == 0)
	{
		...
		ft_builtins(last_list, env_list, flag);
		ft_lstclear(&head);
...
int	make_copy_env_list_char(char **env, t_env **new_env_list)
{
	t_env	*new_env;
	char	*str;
	int		i;
...
int main(int ac, char **av, char **env)
{
	...
	head = NULL;
	env_list = NULL;
	flag = make_copy_env_list_char(env, &env_list);
